...
class EngageController extends Controller
{
  ...
  public function addAccount($username)
  {
    ...
    if(empty(TwitterAccount::where('twitter_id','=',$response[0]->user_id)->first()))
    {
      ...
      $this->updateAccountLog($username);
      ...
      TwitterAccount::where('twitter_id','=', $response[0]->user_id)->update(['is_account' => '1', 'account_id' => Auth::user()->id]);
      $this->updateAccountLog($username);
      return response()->json([
        'status' =>200,
        'message' => 'berhasil menyambungkan akun'
  ...
  public function updateAccountLog($username)
  {
    $result = shell_exec("python " . public_path() . "\API\SearchTwitter.py " . $username);
    $response = json_decode($result);
    $twitterAccountLog = new TwitterAccountLog;
