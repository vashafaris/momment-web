...
#     user = extractor.get_user(username)
    ...
    tweets = extractor.user_timeline(id = username, count = 1, include_rts = False, tweet_mode='extended')
    for tweet in tweets :
        try :
            tweet_media = None
        ...
        if trigger == 0 :
            c.execute("INSERT INTO twitter_tweets VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
                        (tweet_id, tweet_created, tweet_content, tweet_media, location, retweet_count, favorite_count, account_id, reply_status_id, reply_user_id, reply_screen_name, created_at))
            conn.commit()
        else :
...
#     counter = 0
#     repliesCount = pageSoup.find("span",{"class":"ProfileTweet-actionCountForPresentation"})
#     repliesCount = repliesCount.text
    
#     browser.quit()
    
# #     for i in tweetList :

# findReplies("ridwankamil", "1072013199841492992")
